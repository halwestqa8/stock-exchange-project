...
class ReportManagementScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      body: Row(children: [
        // ── Sidebar ──
        const StaffSidebar(activeRoute: '/reports'),

        // ── Main Content ──
        Expanded(child: Padding(
          padding: const EdgeInsets.all(28),
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text(L10n.of(context)!.reportQueue, style: tt.headlineLarge),
            const SizedBox(height: 4),
            Text(L10n.of(context)!.reportQueueSubtitle, style: tt.bodyMedium?.copyWith(color: AppTheme.muted)),
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return Container(
                        ...
                        decoration: BoxDecoration(
                          color: AppTheme.card, borderRadius: BorderRadius.circular(18), border: Border.all(color: AppTheme.border),
                        ),
                        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Row(children: [
                            Container(
                              ...
                              decoration: BoxDecoration(
                                color: isOpen ? AppTheme.amberLight : AppTheme.tealLight,
                          ...
                          Text('${L10n.of(context)!.customerLabel}:', style: tt.labelLarge),
                          const SizedBox(height: 4),
                          Container(
                          ...
                          if (!isOpen && r['staff_response'] != null) ...[
                            ...
                            Text(L10n.of(context)!.staffResponseLabel, style: tt.labelLarge),
                            const SizedBox(height: 4),
                            Container(
                              width: double.infinity,
                              padding: const EdgeInsets.all(12),
                              decoration: BoxDecoration(color: AppTheme.tealLight, borderRadius: BorderRadius.circular(10)),
                              child: Text(r['staff_response'], style: const TextStyle(fontSize: 13, color: AppTheme.ink, height: 1.5)),
                            ),
                          ],
                          if (isOpen) ...[
                            ...
                            Row(children: [
                              Expanded(child: ElevatedButton(
                                onPressed: () async {
                                  await ref.read(apiClientProvider).respondToReport(r['id'], 'Resolved by staff', 'resolved');
                                  ref.invalidate(reportsProvider);
                                },
                                style: ElevatedButton.styleFrom(backgroundColor: AppTheme.teal),
                                child: Text(L10n.of(context)!.resolveBtn),
                              )),
                              const SizedBox(width: 8),
                              Expanded(child: OutlinedButton(
                                onPressed: () async {
                                  await ref.read(apiClientProvider).respondToReport(r['id'], 'Rejected by staff', 'rejected');
                                  ref.invalidate(reportsProvider);
                                },
                                style: OutlinedButton.styleFrom(foregroundColor: AppTheme.red, side: const BorderSide(color: AppTheme.red)),
                                child: Text(L10n.of(context)!.rejectBtn),
                              )),
                            ]),
                          ],
                        ]),
